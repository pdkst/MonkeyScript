...
class DanmuPack {
	...
	constructor(data, offset = 0) {
		...
		this.packetLength = this.dataView.getUint32(
			DanmuPack.packageSettings.packetOffset
		)
		this.headerLength = this.dataView.getInt16(
			DanmuPack.packageSettings.headerOffset
		)
		this.version = this.dataView.getInt16(
			DanmuPack.packageSettings.versionOffset
		)
		this.operator = this.dataView.getUint32(
			DanmuPack.packageSettings.operationOffset
		)
		this.sequence = this.dataView.getUint32(
			DanmuPack.packageSettings.seqOffset
		)
	 ...
	 */
	zlibInflate() {
		const inflate = new Zlib.Inflate(new Uint8Array(this.value), {
			resize: true,
		})
	...
	static packBuffer(bodyBuffer, operation) {
		...
		const headerBuf = new ArrayBuffer(
			DanmuPack.packageSettings.rawHeaderLen
		)
		const headerView = new DataView(headerBuf, 0)
		headerView.setInt32(
			DanmuPack.packageSettings.packetOffset,
			DanmuPack.packageSettings.rawHeaderLen + bodyLength
		)
		headerView.setInt16(
			DanmuPack.packageSettings.headerOffset,
			DanmuPack.packageSettings.rawHeaderLen
	...
	static getDecoder(isUseful) {
		...
		if (unsafeWindow['TextDecoder'] && isUseful) {
			...
			return {
				decode: function (buf) {
					let fromCharString = String.fromCharCode.apply(
						null,
						new Uint8Array(buf)
					)
					return decodeURIComponent(
						unsafeWindow.escape(fromCharString)
					)
				},
			}
		}
...
class DanmuPackIterator {
	...
	hasNext() {
		...
		return (
			!!current &&
			current.offset + current.packetLength < current.data.byteLength
		)
	}
			...
			switch (danmuPack.operator) {
				...
				case Operation.SEND_MSG_REPLY:
					...
					for (let pack of iterator) {
						if (
							pack.version ===
							WebsocketBodyProtocol.VERSION_DEFLATE
						) {
							// inflated message
